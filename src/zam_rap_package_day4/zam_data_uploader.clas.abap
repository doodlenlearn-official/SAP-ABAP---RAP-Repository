CLASS zam_data_uploader DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zam_data_uploader IMPLEMENTATION.


METHOD fill_master_data.
    data : lt_bp type table of zam_bpa,
           lt_prod type table of zam_product.
   ...
   append value #(
    ...
    append value #(
     ...
     insert zam_bpa from table @lt_bp.
     insert zam_product from table @lt_prod.
  ENDMETHOD.
  ...
  METHOD fill_transaction_data.
    data : o_rand type REF TO cl_abap_random_int,
           ...
           lt_so type table of zam_so_hdr,
           lt_so_i type table of zam_so_item.
    ...
    select * from zam_bpa into table @data(lt_bpa).
    select * from zam_product into table @data(lt_prod).
    ...
    insert zam_so_hdr from table @lt_so.
    insert zam_so_item from table @lt_so_i.

  ENDMETHOD.

  METHOD flush.
    delete from : zam_bpa, zam_product, zam_so_hdr, zam_so_item.
  ENDMETHOD.
